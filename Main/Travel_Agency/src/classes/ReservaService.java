...
public class ReservaService {
    ...
    public static Hotel encontraMelhorHotel(List<Hotel> hoteisDisponiveis, String destino, int estrelasMinimas, int precoMax, int reserva){ // recebe a lista de hoteis disponíveis no momento, a cidade
        ...
        for(Hotel hotel:hoteisDisponiveis){ // para cada hotel disponível
            if(hotel.getHotelLocation().equals(destino) && hotel.getVacancyAmount() <= reserva && hotel.getHotelStars() >= estrelasMinimas && hotel.getNightCost() < menorPreco){ // se o hotel fica na cidade desejada, tem pelo menos o
    ...
    public static List<Flight> encontrarMelhorVoo(List<Flight> voosDisponiveis, String origem, String destino, int precoMax, int assentos) { // recebe a lista de voos disponíveis, a cidade de saída e a
        ...
        for (Flight voo1 : mapaVoos.getOrDefault(origem, Collections.emptyList())) { // para cada voo com a origem do cliente no mapa de voos 
            ...
            if (voo1.getDestination().equals(destino) && voo1.getEmptySeats() <= assentos && voo1.getFlightPrice() < melhorPreco) { // se o destino desse voo é o destino do cliente e o preço do voo é o menor até agora
                ...
                for (Flight voo2 : mapaVoos.getOrDefault(voo1.getDestination(), Collections.emptyList())) { // para cada voo com a origem sendo o destino do voo 1
                    if (voo2.getDestination().equals(destino) && voo1.getEmptySeats() <= assentos && voo2.getEmptySeats() <= assentos && (voo1.getFlightPrice() + voo2.getFlightPrice()) < melhorPreco) { // se o destino do voo 2 é o destino do cliente e os preços do voo 1 e do voo 2
                        ...
                        for (Flight voo3 : mapaVoos.getOrDefault(voo2.getDestination(), Collections.emptyList())) { // para cada voo com a origem sendo o destino do voo 2
                            if (voo3.getDestination().equals(destino) && voo1.getEmptySeats() <= assentos && voo2.getEmptySeats() <= assentos && voo3.getEmptySeats() <= assentos &&(voo1.getFlightPrice() + voo2.getFlightPrice() + voo3.getFlightPrice()) < melhorPreco) { // se o destino do voo 3 é o destino do cliente e os preços
